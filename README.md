...
**SVM（支持向量机）参数搜索：**
- **搜索参数**：`C`（正则化强度）、`gamma`（RBF核参数）
- **默认搜索范围**：
  - `C`: [0.1, 1.0, 10.0, 100.0]
  - `gamma`: ['scale', 0.001, 0.0001]
- **最佳参数示例**：C=10.0, gamma='scale'（CV准确率：80.71% ± 1.45%）

**逻辑回归参数搜索：**
- **搜索参数**：`C`（正则化强度）、`penalty`（正则化类型）、`solver`（求解器）
- **默认搜索范围**：
  - `C`: [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]
  - `penalty`: ['l1', 'l2']
  - `solver`: ['liblinear', 'lbfgs', 'saga']
- **最佳参数示例**：C=0.1, penalty='l1', solver='liblinear'（CV准确率：77.94% ± 1.35%）

**随机森林参数搜索：**
- **搜索参数**：`n_estimators`（树数量）、`max_depth`（最大深度）、`min_samples_split`（最小分裂样本数）、`min_samples_leaf`（叶子节点最小样本数）、`max_features`（特征选择策略）
- **默认搜索范围**：
...
- **最佳参数示例**：n_estimators=500, max_depth=15, min_samples_split=20, min_samples_leaf=2, max_features='sqrt'（CV准确率：74.30% ± 1.15%）
...
**参数搜索性能：**
- 搜索过程使用并行计算（默认 `--n-jobs 16`），可显著加速搜索
