...
支持三种集成方式：
    - 投票（Voting）：硬投票或软投票
    - 堆叠（Stacking）：使用元学习器组合基学习器
    - AdaBoost：自适应提升算法，迭代训练弱学习器
"""
...
class AdaBoostEnsemble:
    ...
    使用自适应提升算法进行集成学习。通过迭代训练弱学习器，
    根据错误率调整样本权重和模型权重，最终使用加权投票进行预测。
    
    参数:
        base_models: 基模型列表（将作为弱学习器使用）
        base_model_names: 基模型名称列表（可选）
        n_estimators: 迭代次数（默认: 50）
        learning_rate: 学习率，控制每个弱学习器的贡献（默认: 1.0）
        random_state: 随机种子
    """
    
    def __init__(self, base_models, base_model_names=None, n_estimators=50, 
    ...
    def _get_logger(self):
        """获取logger，如果不存在则创建一个默认的"""
        if self.logger is None:
            self.logger = Logger(name="adaboost_ensemble")
        return self.logger
    
    def _get_base_predictions(self, model, X):
        """获取基模型的预测
    ...
    def _get_base_proba(self, model, X):
        """获取基模型的预测概率
    ...
    def fit(self, X_train, y_train):
        ...
        """
        logger = self._get_logger()
        logger.info("🔧 开始训练 AdaBoost 集成模型...")
        logger.info(f"   基模型数量: {len(self.base_models)}")
        logger.info(f"   迭代次数: {self.n_estimators}")
        ...
        for t in range(self.n_estimators):
            # 选择基模型（循环使用）
            base_model_idx = t % len(self.base_models)
            base_model = self.base_models[base_model_idx]
            base_model_name = self.base_model_names[base_model_idx]
            
            # 克隆模型
            from sklearn.base import clone
            estimator = clone(base_model)
            
            # 使用加权样本训练（如果模型支持 sample_weight）
            try:
                estimator.fit(X_train, y_train, sample_weight=sample_weights)
            except TypeError:
                # 如果模型不支持 sample_weight，使用重采样
                # 根据权重进行加权采样
            ...
            # 在训练集上预测
            y_pred = self._get_base_predictions(estimator, X_train)
            
            # 计算加权错误率
            incorrect = (y_pred != y_train)
            error = np.sum(sample_weights * incorrect) / np.sum(sample_weights)
            
            # 如果错误率 >= 0.5，跳过这个估计器（或使用较小的权重）
            if error >= 0.5:
                logger.warning(f"   Iteration {t+1}: {base_model_name} 错误率过高 ({error:.4f})，跳过")
                continue
            
            # 计算估计器权重
            estimator_weight = self.learning_rate * np.log((1.0 - error) / (error + 1e-10))
            ...
            if (t + 1) % 10 == 0 or t == 0:
                logger.info(f"   Iteration {t+1}/{self.n_estimators}: {base_model_name}, "
                          f"错误率={error:.4f}, 权重={estimator_weight:.4f}")
        
        logger.info(f"✅ AdaBoost 集成模型训练完成，共训练 {len(self.estimators_)} 个弱学习器")
        
        # 显示权重统计
        if self.estimator_weights_:
            weights_array = np.array(self.estimator_weights_)
            logger.info(f"📊 估计器权重统计:")
            logger.info(f"   平均权重: {np.mean(weights_array):.4f}")
            logger.info(f"   最大权重: {np.max(weights_array):.4f}")
            logger.info(f"   最小权重: {np.min(weights_array):.4f}")
    ...
    def predict(self, X):
        ...
        # 获取所有估计器的预测
        predictions = np.array([self._get_base_predictions(est, X) for est in self.estimators_])
    ...
    def predict_proba(self, X):
        ...
        for est, weight in zip(self.estimators_, weights):
            proba = self._get_base_proba(est, X)
            probas.append(proba * weight)
...
def build_parser():
    """构建命令行参数解析器"""
    parser = argparse.ArgumentParser(description="集成学习投票预测脚本", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    
    # 模型路径参数
    parser.add_argument('--lr-model', type=str, default='runs/sklearn_lr_20251112-195315/best.joblib', help='逻辑回归模型路径')
    parser.add_argument('--rf-model', type=str, default='runs/sklearn_rf_20251113-085457/best.joblib', help='随机森林模型路径')
    parser.add_argument('--svm-model', type=str, default='runs/sklearn_svm_20251112-195558/best.joblib', help='SVM 模型路径')
    ...
    # 投票参数
    parser.add_argument(
        '--method', type=str, choices=['voting', 'stacking', 'adaboost'],
        default='voting', help='集成方法：voting（投票）、stacking（堆叠）或 adaboost（自适应提升）'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--adaboost-n-estimators', type=int, default=50,
        help='AdaBoost 迭代次数（默认: 50）'
    )
    parser.add_argument(
        '--adaboost-learning-rate', type=float, default=1.0,
        help='AdaBoost 学习率（默认: 1.0）'
    )
...
def generate_visualizations(ensemble, datasets, results, save_dir, logger):
    ...
    try:
        ...
        if has_pr_data:
            ...
            elif isinstance(ensemble, AdaBoostEnsemble):
                method_name = "AdaBoost"
            else:
                method_name = "投票（{}）".format("硬" if getattr(ensemble, 'voting', 'soft') == 'hard' else "软")
            plot_split_metrics(
                results,
                metric='pr',
...
def main():
    ...
    try:
        ...
        if args.method == 'voting':
            ...
            cfg_lines.append("迭代次数: {}".format(args.adaboost_n_estimators))
            cfg_lines.append("学习率: {}".format(args.adaboost_learning_rate))
        ...
        if args.method == 'voting':
            ...
            logger.info("🔧 创建 AdaBoost 集成模型...")
            ensemble = AdaBoostEnsemble(
                base_models=models,
                base_model_names=model_names,
                n_estimators=args.adaboost_n_estimators,
            ...
            X_train, y_train = datasets.get('train', (None, None))
            if X_train is None or y_train is None:
                raise ValueError("训练集不存在，无法训练 AdaBoost 模型")
            
            ensemble.fit(X_train, y_train)
            logger.info("✅ AdaBoost 集成模型训练完成")
        ...
        results_data = {
            ...
            'adaboost_n_estimators': args.adaboost_n_estimators if args.method == 'adaboost' else None,
            'adaboost_learning_rate': args.adaboost_learning_rate if args.method == 'adaboost' else None,
            'model_paths': {
