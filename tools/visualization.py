...
def plot_logistic_regression_coefficients(model, feature_names=None, top_k=20, title=None, save_path=None):
    """绘制逻辑回归的特征系数条形图。
    
    参数:
        model: 已训练的逻辑回归模型（sklearn LogisticRegression）
        feature_names: 特征名称列表（可选）
        top_k: 展示的最重要特征数量（按绝对值排序）
        title: 图标题（可选）
        save_path: 图像保存路径（可选）
    ...
    if model is None or not hasattr(model, "coef_"):
        raise ValueError("model 必须是已训练的逻辑回归模型。")
    ...
    if feature_names is None:
        display_names = ["Feature {}".format(idx) for idx in top_indices]
    else:
        display_names = [feature_names[idx] for idx in top_indices]
    
    plt.figure(figsize=(8, max(4, top_k * 0.4)))
    colors = ["#4c78a8" if coefficients[idx] >= 0 else "#f58518" for idx in top_indices]
    plt.barh(range(len(top_indices)), coefficients[top_indices], color=colors)
    plt.yticks(range(len(top_indices)), display_names)
    plt.axvline(0, color="black", linewidth=0.8)
    if title:
        plt.title(title)
    else:
        plt.title("逻辑回归特征系数（前 {} 项）".format(top_k))
    plt.xlabel("系数")
    plt.gca().invert_yaxis()
    plt.grid(axis='x', linestyle='--', alpha=0.3)
    
    output_path = _prepare_output_path(save_path, "logistic_coefficients.png")
    plt.tight_layout()
    plt.savefig(output_path, dpi=200)
    plt.close()
    return output_path
...
def plot_random_forest_importance(model, feature_names=None, top_k=20, title=None, save_path=None):
    """绘制随机森林的特征重要性条形图。
    
    参数:
        model: 已训练的随机森林模型（sklearn RandomForestClassifier）
        feature_names: 特征名称列表（可选）
        top_k: 展示的最重要特征数量
        title: 图标题（可选）
        save_path: 图像保存路径（可选）
    ...
    if model is None or not hasattr(model, "feature_importances_"):
        raise ValueError("model 必须是已训练的随机森林模型。")
    ...
    plt.figure(figsize=(8, max(4, top_k * 0.4)))
    plt.barh(range(len(top_indices)), importance[top_indices], color="#54a24b")
    plt.yticks(range(len(top_indices)), display_names)
    if title:
        plt.title(title)
    else:
        plt.title("随机森林特征重要性（前 {} 项）".format(top_k))
    plt.xlabel("重要性")
    plt.gca().invert_yaxis()
    plt.grid(axis='x', linestyle='--', alpha=0.3)
    
    output_path = _prepare_output_path(save_path, "random_forest_importance.png")
    plt.tight_layout()
    plt.savefig(output_path, dpi=200)
    plt.close()
    return output_path
...
def visualize_svm_results(results, save_dir=None, metric='pr'):
    """可视化 SVM 的 PR 曲线。
    
    参数:
        results: SVM 训练结果字典，与 run_sklearn_training 返回值一致
        save_dir: 保存目录（可选），默认写入当前目录
        metric: 输出文件名标识（默认: "pr"）
    ...
    filename = "svm_pr_curve.png" if metric == 'pr' else "svm_{}.png".format(metric)
    return plot_split_metrics(
        results,
        metric='pr',
        title="SVM PR 曲线",
...
def visualize_logistic_regression_results(results, model=None, feature_names=None, save_dir=None, metric='pr', top_k=20):
    """可视化逻辑回归的 PR 曲线与特征系数。
    
    参数:
        results: 训练结果字典，与 run_sklearn_training 返回值一致
        model: 已训练的逻辑回归模型（可选）
        feature_names: 特征名称列表（可选）
        save_dir: 保存目录（可选）
        metric: 输出文件名标识（默认: "pr"）
    ...
    save_dir = Path(save_dir)
    outputs = {}
    filename = "logreg_pr_curve.png" if metric == 'pr' else "logreg_{}.png".format(metric)
    outputs['pr_curve'] = plot_split_metrics(
        results,
        metric='pr',
        title="逻辑回归 PR 曲线",
        save_path=save_dir / filename
    ...
    if model is not None:
        outputs['coefficients'] = plot_logistic_regression_coefficients(
            model,
            feature_names=feature_names,
            top_k=top_k,
            title="逻辑回归特征系数（前 {} 项）".format(top_k),
            save_path=save_dir / "logreg_coefficients.png"
        )
    return outputs


def visualize_random_forest_results(results, model=None, feature_names=None, save_dir=None, metric='pr', top_k=20):
    """可视化随机森林的 PR 曲线与特征重要性。
    ...
    save_dir = Path(save_dir)
    outputs = {}
    filename = "random_forest_pr_curve.png" if metric == 'pr' else "random_forest_{}.png".format(metric)
    outputs['pr_curve'] = plot_split_metrics(
        results,
        metric='pr',
        title="随机森林 PR 曲线",
    ...
    if model is not None:
        outputs['importance'] = plot_random_forest_importance(
            model,
            feature_names=feature_names,
            top_k=top_k,
            title="随机森林特征重要性（前 {} 项）".format(top_k),
